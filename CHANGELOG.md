...
- **`$short-drama-novel-analyze`：长篇原著分析技能**。把小说、连载或多集散稿拆成可追溯的
  分析层：章节索引（脚本）、开篇三章深度拆解、逐章功能提取、剧情单元与节奏情绪聚合、
  人物与设定候选、改编价值判定与分集候选。产出全部落在 `项目开发/原著分析/`，
  归本技能所有。**它只产候选，不产契约**——`项目开发/adaptation-map.jsonl`、创作简报与
  分集地图仍归 `$short-drama-develop`，分析可以被推翻，已接受的改编承诺不能。
  管道在开篇三章后停靠一次，让创作者先决定值不值得全量拆。
- **`scripts/novel_index.py`：唯一切片真源**。章节边界只算一次并绑定 `content_sha256`，
  供后续每个阶段共用；识别中文数字章号（含 千 / 两，覆盖千章以上连载），剔除开头目录块，
  标注多卷重号，并提供 `verify` 与 `coverage` 两个确定性检查。此前若让每个阶段各跑一次
  正则，同一章会按不同边界被分析和聚合，而且不会有任何报错。
- **`$short-drama-voice-prompts`：角色音色提示词技能**。把 `$short-drama-assets` 已接受的
  `voice_direction`（AST-07）投影成可复制的音色、选角与试音提示词，产出
  `设定集/voice-prompt-specs.jsonl` 与 `设定集/voice-prompts.md`。声音身份仍归 assets、
  单句表演仍归 write 的配音本；本技能只负责“把已接受的声音说清楚给别人复制”。
  **仍不生成音频，也不调用任何语音服务。**
- **输出语言契约落地**。`short-drama.json#/language` 此前只在 `init` 写入、之后没有任何
  地方读取；现在它是创作者可读产物的权威，并新增
  `#/format/prompt_language`（默认 `en`）管提示词正文。两个字段在 `init` 校验格式、
  由 `status` 一并报出，技能不再各自猜默认值。**记为新增**：默认行为与此前一致
  （中文项目仍出中文），创作者可覆盖任一字段。

### 变更

- **新增两个技能目录，安装时必须与其余技能保持同级**。套件清单从 8 个公共技能变为 10 个；
