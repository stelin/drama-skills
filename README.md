...
Seedance、Seedream、GPT Image 2、MiniMax H3 视频、MiniMax Music 与 codex-imagegen（本机 codex 登录态出图，零 API key）的
可选 adapter，但项目文件和其他 Skill 不绑定供应商。
...
做横屏电影长片时，在《视觉设定.md》声明观看契约，竖屏短剧的默认（切镜密度、避让区、首镜不给空景）就整体让位；
血亲/派生资产、设定板版式卡、交付分组与方言标记对账、codex 零 key 出图也都在这条路径上。完整样例是
[《冻河》第一本](examples/creator-first-film/)，流程见 [电影长片全流程指引](docs/film-workflow.md)，
合并设计见 [docs/film-merge-design.md](docs/film-merge-design.md)。第一次上手、想按步骤在 Codex 里从安装走到
出图出片，看 [Codex 操作步骤](docs/codex-usage-guide.md)；用 Codex 桌面 app、手上已有一部小说，看
[Codex 桌面端操作手册](docs/codex-desktop-guide.md)。
...
| `short-drama-produce` | 展示有边界的图片/视频/TTS/音乐任务，取得本次明确确认后通过外部 adapter 执行并记录结果；可选支持 Seedance、Seedream、GPT Image 2、MiniMax H3 视频、MiniMax Music 与 codex-imagegen |
