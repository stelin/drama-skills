...
**`structural_invariant` AST-01：** 每条出现记录都要指向准确的剧本文件、`hash`、字段、
剧本段 ID 和场次。行号只能帮助阅读，不能代替稳定来源。
...
| 内容 | 字段用途 | 写法 |
|---|---|---|
| 剧本段和场次位置 | `source_ref` | 负责人、文件、`hash` 和字段；不复制整段权威文本 |
| 剧本明示的可见事实 | `derived_projection` | 简短摘义，并指回准确字段 |
| 拟绑定的资产 | `derived_annotation` | 只保留提案；资产决定记录反向引用本条出现记录，并拥有最终决定 |
| 连续性变化 | `source_ref` | 指向资产技能负责的变化记录及其文件、`hash` 和字段 |

不要把推测伪装成可见事实。可以直接推出的结果也要说明依据，例如“匣盖被推开”可以
记为 `open`；“顾禾很内疚”不能只因她低头就升级成资产事实。

出现记录不要用整文件 `hash` 反向引用尚未建立的决定记录。决定尚未创建时只写
`decision_locator`；决定创建后，由决定记录指向出现记录。未来的连续性变化同样先写
定位信息。这样不会让两个文件互相等待对方的最终 `hash`。
...
不要为了“让流程继续”做低把握的猜测。保留待定项，可以防止错误身份被下游放大成
参考图、分镜和视频提示词。
