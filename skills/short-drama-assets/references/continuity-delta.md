...
不足以解释整栋渡站永久断电。**`reviewed_invariant` CON-02：** 独立审查者要结合剧本
...
下一集若直接写顾禾手持号牌，必须有归还记录或由创作者接受的修订，不能悄悄“对齐”。
...
- 修改伤势开始的剧本段：把该造型、变化记录及依赖它的提示词、镜头、运动规格和审查标为 `stale`。
- 只修改道具显示名，ID 和事实不变：通常只更新显示文本，不重做无关分镜。
- 修改镜头构图：不会让资产文件自动变为 `stale`。
