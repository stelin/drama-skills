...
1. **选型与验收**：用什么判断依据挑一条参考、判一次克隆结果算不算这个角色；
...
与参考图同一套纪律（见核心的 [reference-roles.md](../../short-drama/references/reference-roles.md)）：
绑定要写清这次参考它的**什么**，以及哪些内容**不能**跟着来（`AST-08`）。
...
参考音频与授权说明属于创作者输入，放在 `输入/` 下按整文件 hash 绑定。
**媒体本身不进 设定集，也不进交付包**，只进引用与 hash。

## 选型判断依据

`selection_criteria` 是三到五条可听的判断依据，用来**挑**一条参考、或**判**一次克隆结果
算不算这个角色（`AST-12`）。它不生成声音，也不替代参考。

从六类里挑，不要写满：音区（相对位置而非 Hz——「比同剧其他成年男性低半档」）、
质感、年龄与体量印象、节奏、口音与语域、可重复的标记性习惯（至多一条）。

每条带一个 `counter_example`，写清什么算过头。判断依据没有上界时，试音会一轮比一轮夸张，
克隆结果也没法判到底算不算过。

情绪词不是判断依据。锚点里出现「愤怒」「悲伤」「紧张」，说明写的是表演不是身份；
换成产生那个印象的可听特征。
...
区分判断依据：A 句尾收住不拖长；B 句尾常带一段下滑的余音。
```
...
- `cause`：由哪条已接受的剧本事实引起；
- `validity`：从哪里生效到哪里失效；
- `delta`：换了哪条参考、或相对基础身份改了哪几条判断依据，其余继承。

短暂的一场变化不建变体，交给表演。判断依据和 Look 一样：跨多个场次持续、会重复使用、
...
记录先落进文件、由创作者确认，再由创作者自行决定送去哪里。

常见失败：

| 失败 | 症状 | 怎么改 |
|---|---|---|
| 用文字冒充音色 | 没有参考，却写了一整段音色描述当身份 | 保持 `reference: null` 并标「待选型」，判断依据归判断依据 |
...
| 两个角色撞声 | 两条参考互换名字也说得通 | 补区分判断依据；补不出就换参考 |
