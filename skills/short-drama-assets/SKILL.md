...
4. 写最小可识别设定：统一使用创作者语言的 `识别锚点`、项目提示词语言的 `执行锚点`、本集状态和变化原因。执行锚点是一条能离开项目单独描述身份/地理/形制的最小可见短语，不写镜头瞬态；会持续限制后续动作的身体—物件—空间关系写进有生效范围的状态，不能当成一次姿势略过。
5. 检查造型/伤势、持物/所有权、道具状态、地点时段/天气/光态与跨集 outgoing。
6. 给跨镜出现且观众看得出不一致的可见事实写一条连续性锁，其余保持识别锚点即可。
7. 整集请求自动处理完整集，末端一次回报关键身份决定和未决指代。

## 内容要求

- 每项先写什么不变使它仍是同一资产，再写本集变化。
- 识别锚点必须可见、可生成、可比较，不用空泛质量词替代事实。
- 每个人物、地点、道具都有一条非空且不与其他资产重复的执行锚点；不能只写项目专名、`same character` 或“与上一镜相同”。远景也能辨认的剪影、发型、服装色块优先，脸部细节只在适合景别时由分镜补充。
...
所有明确出场都已复用、新建、变体化或标为真实未决；每项已有可供无参考图执行的最小执行锚点，
需要保持的视觉事实与状态变化已写入唯一
