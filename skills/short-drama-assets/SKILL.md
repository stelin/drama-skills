...
7. 项目提示词语言不是中文，而这一条会在提示词正文里被直接叫到名字时，写一行 `画面代称：<正文里的拼写>`。
...
- 「画面代称」是下游把提示词正文和本条目对上的唯一凭据：分镜的机械核对靠它判断关键帧里点名的人是谁。
  同一个名字全集只用一个拼写（`AST-10` 的同一条要求跨语言同样成立）；多个写法用 `、` 分隔。条目自身的中文名称始终算一个代称，
  正文从不点名的地点或道具不必写这一行。
