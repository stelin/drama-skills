...
| 笼统的美词是否挤压锚点 | `craft_default` | 指出可删词与缺失的可见事实 |
...
为已接受锚点找到剪影能表达的写法（发际形状→发型剪影缺口、旧伤→轮廓上的缺口、体态→肩背
重心），而不是新增、替换或悄悄删掉锚点。找不到可靠通道时记 `creator_decision_needed`，退回
资产或创作者，不擅自换一个更好画的身份。

### 模板字段不得覆盖身份锚点

**`reviewed_invariant`**：模板字段不得覆盖已接受的身份锚点；当模板默认值与已接受身份冲突时，
**身份优先，模板让位**。

合成例：角色甲的已接受身份是五十岁上下、体格厚重、肩背略沉。若人物板模板对每个角色机械
追加“九头身黄金比例、极度修长的双腿”，这条默认字段就把角色甲改写成了另一个人，而且因为它
...
| 检查 | 分类 | 证据 |
|---|---|---|
| 形态转写是否只落在词汇层 | `reviewed_invariant` | 形态卡字段与提示词对应句 |
