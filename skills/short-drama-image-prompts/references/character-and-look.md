...
资产阶段声明了 `派生自` 的人物（子女、变异形态、双胞胎……），身份板的写法只多两件事：

- `参考：` 用上游人物的定稿图各占一个 `REF-` 槽位，`用途：身份`；`控制：` 逐字写声明的继承项
  （通常是 2–4 项骨相），`不得控制：` 逐字写不继承项，再加构图与表情。上游没有定稿图时写
  「待上游定稿：人物「…」」，不虚构 `REF-`，也不先凭文字出图——先出来的脸与之后定稿的父母无关。
- 可复制正文把继承写成可见事实（眉弓怎样、鼻梁怎样、下颌角怎样），紧接着写分化（年龄、脸部
  脂肪、发型、皮肤状态、体型），不写「像父亲」「有家族感」。双胞胎/克隆之外的派生条目，正文里
  至少有一处结构性差异，否则执行端拿着父亲的图会画出父亲。

**`reviewed_invariant`**（资产技能 `AST-14`）：继承项超过四项、包含年龄或 Look 层内容、
或非双胞胎写成同一张脸，都是缺陷。版式选择见 [设定板版式卡](character-sheet-layouts.md)。
