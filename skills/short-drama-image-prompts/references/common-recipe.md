...
| 复用几何 | 后续镜头最怕哪里走样？ | 方位、尺度、入口、握持面、结构关系 |
...
- `character_sheet`：身份锚点绑定一个造型版本，选择便于识别的构图；没有文字表面时删除文字字段。
...
| 资产来源政策 | 允许的呈现方法 | 要求 |
|---|---|---|
| `exact_readable` | `readable` 或 `postproduction` | 前者逐字引用；后者保留文字区域与精确文字引用 |
...
`postproduction` 时可预留文字区域，不得用姓名、伪签名或自行设计的笔画冒充正式字形。
若人物的接收或反转依赖这项文字，不得一面保留空白文字区域、一面让人物认出签名；候选文本
保留 `<正式签名字形-ref>` 待定引用，并把涉及可读识别的帧/提示词范围标为 `blocked`，直到
字形或已接受的合成处理真正到位。
...
- `exact_readable`：只允许批准的文字表面和逐字内容；旧版本、冲突号码、其他标牌或
  字幕不能借参考图进入生成。
...
`reference_observation_ref` 与 `unresolved_risks`。数组重排或插入新参考不能改变已有字段语义。
常见用途包括身份、造型/状态、
...
2. **内容**：独立审查者引用资产字段和提示词句子，判断身份、场景地理和尺度是否保持，
   不能只说“不够高级”。
3. **默认**：把笼统的词提前、堆砌或重复视作可改进警告；只在必需事实缺失时才升级。
