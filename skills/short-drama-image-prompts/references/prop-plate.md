...
- **`craft_default`**：尺度先定档位再写参照。资产条目可以声明 `尺度：手持级 | 桌面级 | 家具级`
  （或项目自定义档位）；声明之后，本道具每条可复制正文都带上对应的尺度短语——按
  `#/format/prompt_language` 写 `handheld scale` / `tabletop scale` / `furniture scale`，或该语言里的档位原词。
  **`IMG-16 · structural_invariant`**：资产条目声明了档位而正文没有短语，`creator_markdown_check.py`
  报错——手持道具被画成家具尺寸是最常见、也最难从单张图上看出来的事故。
- **`taste_option`**：单件档案照版式。要一张能被反复贴进各镜头的道具参考时，可选竖构图（3:4）
  单件档案照：只展示这一件，最有识别力的四分之三角或接近正视，长条形道具走对角线，主体占画
  六到八成，不裁切握持端、刃口、开合结构与关键文字；纯白或浅灰无缝背景、高调柔和中性光、只留
  轻微接触阴影；无人、无手、无场景杂物；高光服从材质，不做油亮塑料化。它与白底可抠、排除人手
  的做法是同一件事的两种写法，按项目选一种；道具的结构靠单张最有识别力的角度加必要的细节格
  说清，不做正侧背三视。
