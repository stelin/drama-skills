...
3. 先把对应《视觉设定.md》条目的执行锚点原样放进正文，再按“主体与身份锚点 → 当前变体 → 构图/视角 → 光色/材质 → 背景边界 → 禁止项”写完整提示词。
...
- 每个条目的可复制正文包含它所对应人物、地点或道具的执行锚点；组合板包含每个被它控制的资产执行锚点，不能只靠 IMG 标题或项目专名指认对象。
...
每个点名对象都有明确用途、可复制正文、参考边界和禁止项，且相互不矛盾，即完成。若已有分镜，
完成前列出新增、删除或改名条目影响的 `SHOT-...`；本次请求横跨图片提示词与分镜时，自动交回
storyboard owner 刷新可见资产映射，不能只更新一边。实际生成必须
