...
方向未定时，提出少量**机制不同**的候选：改变主角策略、对抗来源、代价或持续冲突方式，而不只是换标题、职业和用词。方向明确时直接深化，不为凑选项制造伪差异。
...
压力来源、场面质感与制作负担；它是可被创作者一句话覆盖的参考，不替代方向选择，也不进入交付物。
...
读取 [episode-design.md](references/episode-design.md)，再复制 [episode-map.jsonl](assets/episode-map.jsonl)。每一集记录进入状态、当集追求、阻力、因果升级、方向性转折、已经回报的铺垫、出去的压力以及下一集必须继承的事实。
...
- **`reviewed_invariant`**：升级是否真实、承诺是否落实等语义义务；只能由独立审查者引用证据判断。
...
- **承诺、引擎、人物压力、升级与铺垫回报**：[story-craft.md](references/story-craft.md)
...
- **已确定题材，需要该题材更常用的压力、场面质感、钩子方向与制作难点**：
