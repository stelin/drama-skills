...
一份 `triage.md`（分诊报告），回答**这本书值不值得花全量拆解的成本**。
...
默认 12 章。书特别长或框架复杂时调大 `--count`，调大要写进快速评估。抽到的章正文极少
（索引里 `char_count` 很小）时往后顺延一章并记录——那多半是卷首页，不是内容。

快速评估只判断**功能与载体**：不写完整情节点表，不写人物表，不产出 `chapters/` 下的文件。
逐章提取的格式要求见 [章节提取](chapter-extraction.md)，这一步不套用。

## 写六件事

`triage.md` 固定六段，缺一段不算快速评估。
...
| `prose_only` | 功能依赖读者与叙述者的关系，屏幕上无法实现 |

写出三类各占抽样里的多少章。这是快速评估里唯一给数字的一段，其余用文字判断。
...
```markdown
> 本快速评估基于抽样：读了 340 章中的 12 章（3.5%），章号见下表。结论是假设，不是结论。
```
...
- **比例的分母是抽样章数**，在文里写出来。
- **候选集条目是 S5 的产物**，快速评估给量级即可。

## 停靠与交接

快速评估写完后向创作者报告：覆盖率、六段结论、全量拆解的预计耗时（按章数粗估）、
