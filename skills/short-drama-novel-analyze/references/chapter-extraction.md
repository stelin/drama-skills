...
5. **待定项**：本章无法判断的指代、时间关系或矛盾，逐条列出。留白比猜测便宜。
...
```
P1 类型 | 客观描述 | 涉及：称谓、物件、地点 | 功能：这一步改变了什么
```

- **类型**取自：`行动`、`对话`、`信息`、`转折`、`铺垫`、`回报`、`状态`。
- **客观描述**是可观察的事实：谁做了什么、说了什么、什么被看见。见下一节。
...
客观描述是这条情节点的主要证据——后面每一次事实回查都落在它上面。所以它只能写
...
谁对谁说了什么、某人出现在哪几章——这类事实写进聚合产物前，必须能回到某条客观描述或原文；
...
下游会按同样的字面形状 grep：格式漂一个字符，聚合阶段就会悄悄漏掉整章。
...
- 客观描述段计数少于 `$P`：有情节点缺客观描述，或字段顺序、分隔符不对。
- `功能：` 计数少于 `$P`：有情节点没回答改变了什么。注意全角冒号——半角会漏。
- 类型超出枚举：自造类型会让 S3 的归类整段落空。
- 载体表缺失：S5 会失去判定 `needs_carrier` 的依据，只能凭印象补。

文件名由 S2 的覆盖率关卡按字面匹配，写歪一个字符会进 `unmatched_files`——
