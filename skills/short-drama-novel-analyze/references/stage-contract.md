...
2. **先恢复事务，再读状态**：定位项目根目录后，先运行 core 的 `scripts/project_tool.py`
   ...
   原样并先处理冲突，不要绕过 WAL、手改状态文件或假定上次写入成功。
...
- **本阶段拥有**：`项目开发/原著分析/` 下的章节索引、逐章提取、剧情单元、节奏情绪、
  人物与设定候选、改编价值评估与分集候选。
- **本阶段继承**：创作者提供的原始材料与授权说明、项目语言、已接受的形式约束。
...
| NVA-05 | reviewed_invariant | A function summary states what a passage does to character choice, information, power or relationship, not what happens in it. |
| NVA-06 | reviewed_invariant | Character merges preserve dramatic role, knowledge scope, relational position and causal bridge; fewer names is not evidence of a good merge. |
| NVA-07 | reviewed_invariant | Adaptation value distinguishes what the screen can show from what only prose can deliver, and names the new carrier for each function it keeps. |
| NVA-08 | reviewed_invariant | Episode candidates are cut on local dramatic result and precise handoff, not on chapter count or word budget. |
| NVA-09 | craft_default | Analyse the opening chapters first and stop for the creator before committing to a full pass. |
| NVA-10 | taste_option | Where to open, which line to keep and which ending to promise remain creator choices; analysis may argue but never blocks. |
