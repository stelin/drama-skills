...
description: 把长篇小说、连载网文或多集散稿拆成可追溯的原著分析：章节索引、开篇拆解、逐章功能提取、剧情单元与节奏聚合、人物与设定归并，最后给出改编价值评估与分集候选，交给 $short-drama-develop 立契约。用户说“导入这本小说”“拆这本书”“分析原著”“这本书能不能改短剧”“先看看开头三章”“把长篇拆成分集候选”，或直接给出小说文件路径时使用。只做只读的结构化分析，不写剧本、不建资产、不生成媒体，也不替创作者决定改编方案。
license: MIT
---
...
3. **有原文，项目已在**：直接进入管道。
4. **已有部分分析**：读 `项目开发/原著分析/_progress.md` 从断点续跑，不重跑已完成阶段。

## 管道

`输入/` 是不可变的创作者输入，本阶段只读它。全部产出落在 `项目开发/原著分析/`。

| 阶段 | 做什么 | 产出 | 停靠 |
|---|---|---|---|
| S0 | 建章节索引（脚本） | `_index.json`、`_progress.md` | 索引有问题就停 |
| S1 | 开篇三章深度拆解 | `章节/第N章-拆解.md` ×3、`开篇评估.md` | **停靠问创作者** |
| S2 | 逐章功能提取 | `章节/第N章-提取.md` | 覆盖率不足就停 |
| S3 | 剧情单元与节奏聚合 | `剧情单元.md`、`节奏与情绪.md` | — |
| S4 | 人物归并与设定 | `人物.md`、`设定.md` | — |
| S5 | 改编价值与分集候选 | `改编价值.md`、`分集候选.jsonl` | 交接 develop |
...
python3 {技能目录}/scripts/novel_index.py index 输入/{原文文件} \
  --out 项目开发/原著分析/_index.json
```

脚本识别阿拉伯数字与中文数字章节标题（含 千 / 两，覆盖千章以上连载），剔除开头的目录块，
标注多卷重号，并给每章绑定 `content_sha256`。它**不做编辑判断**——哪章重要、讲了什么，
是后面阶段的事。

`problems` 非空就停下报告，不要带着错表进 S1。常见三种：章号跳号（缺章或抓错标题）、
正文极少的章（多半抓到了目录残留或卷首页）、无法解析的章号。原文本身没有章节标题时
索引会返回空表，此时与创作者确认按什么切分，把边界写进 `_index.json` 再继续。

原文换了一个字节，`verify` 就会报出来；改了原文必须重建索引，不能沿用旧 span。

```bash
python3 {技能目录}/scripts/novel_index.py verify \
  项目开发/原著分析/_index.json 输入/{原文文件}
```

### S1 开篇三章

短剧的开篇成本最高，也最能提前判断这本书值不值得全量拆。按
[章节提取](references/chapter-extraction.md) 的深度拆解写法处理前三章，然后写
`开篇评估.md`：这本书靠什么留住人、第一个具体承诺出现在哪里、主角第一次做出有代价的
选择是什么、以及改编到竖屏短剧最先要动的是什么。

**这里停靠问创作者**：给出开篇评估与预计耗时（按章数粗估），问是否继续全量拆解。
创作者一开始就明确说「一次跑完」时仍写 `开篇评估.md`，但不停下等待。

停靠时把 `_progress.md` 的状态写成 `paused_after_s1`，断点写「下一步：S2 逐章提取」。
...
每章提取完落到 `章节/第N章-提取.md`。全部落盘后跑覆盖率：

```bash
python3 {技能目录}/scripts/novel_index.py coverage \
  项目开发/原著分析/_index.json 项目开发/原著分析/章节
```

`missing` 非空就补跑缺的章。**不要在覆盖率不足时进入 S3**——聚合会照样产出一份读起来
完整的结果，而缺掉的章不会在任何地方留下痕迹。
...
矛盾的事实。按 [聚合与实体](references/aggregation-and-entities.md) 产出：

- `剧情单元.md`：把情节点归成有始有终的单元，每个单元记录进入状态、冲突、代价与出去状态；
- `节奏与情绪.md`：关键信息如何逐章推进、情绪触动点的铺垫→释放→余波、跨章伏笔与兑现。

### S4 人物与设定

按 [聚合与实体](references/aggregation-and-entities.md) 归并人物（跨章去重、别名归一、
分级），并从提及数据归纳世界规则、力量体系与势力。
...
- `改编价值.md`：哪些单元在竖屏短剧里能直接成立、哪些要换载体、哪些是纯文字快感
  （内心戏、叙述性诡计、长铺垫）在画面上无法兑现；制作负担落在哪里。
- `分集候选.jsonl`：按**局部戏剧结果与精确交接**切出的候选集，不按章号或字数平均切。
  每条带来源 span/hash、承担的功能、以及未决项。
...
最大的三处改编风险。然后交给 `$short-drama-develop`——由它把候选变成
`项目开发/adaptation-map.jsonl` 与改编契约。
...
本技能只拥有 `项目开发/原著分析/` 下的文件：`_index.json`、`_progress.md`、
`章节/*.md`、`开篇评估.md`、`剧情单元.md`、`节奏与情绪.md`、`人物.md`、`设定.md`、
`改编价值.md`、`分集候选.jsonl`。
...
分析产物跟随项目 `short-drama.json#/language`；创作者另有要求时按创作者的。

## 按需加载

- **逐章提取写法、机械自检、并发与串行的同一份要求**：[章节提取](references/chapter-extraction.md)
- **剧情单元、节奏情绪、人物归并与设定归纳**：[聚合与实体](references/aggregation-and-entities.md)
