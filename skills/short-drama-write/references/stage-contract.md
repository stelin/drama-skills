...
2. **先恢复事务，再读状态**：定位项目根目录后，先运行 core 的 `scripts/project_tool.py`
   ...
   原样并先处理冲突，不要绕过 WAL（预写事务日志）、手改状态文件或假定上次写入成功。`status` 中的
...
3. **只通过公开生命周期写入**：负责人用 `publish` 原子发布候选，并给每个外部结构化引用
   ...
   不同动作。每次修订后重新运行适用的结构校验，并让下游刷新旧 hash。打包是最终交付关卡，
   不是接受或审查命令；仍有阻断项时不打包。
...
- **本阶段继承**：已接受的单集契约与已接受事实。开发环节拥有已规划契约时，本阶段只转写
...
题材机制会影响压力来源、可用证据、场面质感与集尾方向，但**选择哪个题材、用哪套钩子方向
属于开发阶段**。本阶段的做法是：

- 已有已接受的题材与钩子方向时，按它执行，并以单集契约为准；
- 没有已接受方向时（例如从想法直接写作），**不要贴题材标签给自己造公式**。改为向创作者
  确认本集要落实什么承诺、观众该获得什么回报，然后按第 5 节的叙事工艺决策规则写；
