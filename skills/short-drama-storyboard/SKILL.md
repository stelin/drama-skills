...
3. 每镜写清唯一职责、来源、时长、景别/机位、起点、唯一主要动作、终点、声音和视觉依据；用
   `起点 → 唯一动作 → 终点` 直接说明人物、双手与持物怎样完成本镜状态转换。
4. 锁定人物朝向、位置、视线、持物、出入口、屏幕方向和必要状态；上游已有会限制动作空间的持续关系时，把关系投影进本镜起点与终点。
5. 冻结关键帧只投影镜头起点；删除终点才出现的文字、动作、姿态或道具状态。
6. 逐镜完成参考准备：在用户提供的输入、项目「制作成果」和文档已指向的其他可见媒体中查找匹配图片；命中就写入
   `REF-...`。一张都没命中时写「无（待补参考图：<本镜缺失的起始帧 / 人物 / 地点 / 道具>）」；只命中一部分时，
   在已有 `REF-...` 后追加「；待补参考图：<仍缺的图片>」。只有创作者明确表示不用图时，才写「无（创作者已明确选择文生视频）」。
...
- 冻结关键帧成稿后，按实际画面反查需要保持身份、造型或地理的可见人物、地点和道具；每项都应由「图片提示词项」或《视觉设定.md》依据覆盖。小比例或背景人物只要需要认出身份仍在范围内，画外人物和不可识别局部不算。
- 「输入参考图」是独立的另一条轴：创作者已提供，或经确认生产且当前可读的实际图片，用不同于 `IMG-...` 的稳定 `REF-...` 槽位引用。每个槽位同时写显式顺序、仅用 `/` 分隔的项目相对路径、中文名称、「控制」和「不得控制」范围；没有时必须区分「待补参考图」和「创作者已明确选择文生视频」，只准备了一部分时则保留已有 `REF-...` 并追加「；待补参考图：……」。不得把提示词条目、计划生成的图或对话中的描述写成已有图片。
- 每个 `REF-...` 必须逐字遵循唯一格式：`REF-<slot>（顺序：<n>）· <项目相对路径>《<中文名称>》（控制：<范围>；不得控制：<范围>）`。注意「顺序：」后的全角冒号和路径前的中点 `·`；不得用 `/` 代替字段分隔符。多图只用中文分号 `；` 连接，部分缺图则在最后一个完整槽位后接 `；待补参考图：<缺口>`。写完后按此格式回读一次再交付。
- 自动绑定不是把所有图都塞进一镜。只选本镜可见、且身份、造型、地理、关键道具状态或起始构图确实需要稳定的图；逐张写清它能控制什么、不得控制什么。
...
完成。本次请求同时修改《图片提示词.md》与《分镜.md》时，当轮刷新受影响镜头的引用；不另建覆盖表。
