...
- 每镜再写一条项目提示词语言的「静态视觉锚点」，冻结关键帧正文以它开头，视频提示词逐字沿用。它包含所有起点可见资产在《视觉设定.md》的执行锚点；过程或终点才出现的资产留给运动正文。真实参考图是补充，不是整镜豁免：它可能只控制构图或其中一个资产。项目内专名、`same character`、`与上一镜相同` 和 `保持已有外观` 都不能替代执行端未收到的外观。
