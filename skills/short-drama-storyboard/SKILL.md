...
- 关键信息的可读性优先于装饰性运镜；竖屏构图优先保证主体和反应可见（《视觉设定.md》声明观看契约为电影长片或横屏剧集时按契约的画幅与景别语言，见 `$short-drama` 的《观看契约》）。
- 镜头 `时长` 写 `Ns`，未定时写 `待定`；本集时长的算术和与目标差值只报告；已接受档案声明原生时长时，未进交付分组的镜头必须落在区间内。
