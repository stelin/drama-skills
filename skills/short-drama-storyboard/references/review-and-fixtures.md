# 分镜完成前检查

这份清单只检查 `分镜.md` 能直接证明的结构事实，把审查注意力留给真正需要读懂戏的判断。
检查发现问题就修正文档，不另建 coverage、QA 或校验结果文件。

## 可直接核对

- 每个 `SHOT-...` ID 唯一，来源场景 ID 能在 `剧本.md` 找到；
- 每个制作相关的动作、对白、VO/OS、SFX、画面文字与连续性要求都由镜头承载，或有明确创作理由；
- 同一来源动作只有一个主要落实镜头，其他镜头只增加反应、细节或重新解释；
- 每镜时长是正数，全剧时长等于可见镜头时长之和；目标时长只用于报告差值，不是质量门槛；
- 起点和终点都写绝对事实，不使用“同上”“保持不变”代替；
- 相邻镜头的站位、朝向、视线、持物、伤势、光态与可读文字连续；
- 每镜先列完整可见资产及 `起点/过程/终点/全程` 范围；逐项与景别、起点、终点和冻结帧反查，小比例或背景人物只要需要认出身份/造型也不能漏，画外人物和不可识别局部不误绑；
- 每镜的视觉来源可追溯：已用的 `IMG-...` 必须是《图片提示词.md》的可见标题，带中文名称、对应资产和控制范围；没有合适条目的资产指向《视觉设定.md》。两类来源的合集必须恰好补齐可见资产，不能只验证已写引用是否合法；
- 提示词条目与真实输入图片分开记录，两者可各自为「无」；只有创作者已提供或经确认生产、且当前可读的实际图片才能声称为「输入参考图」，并用独立稳定的 `REF-...` 槽位、显式顺序、仅用 `/` 分隔的项目相对路径、中文名称、控制与不得控制范围定位；
- 每个视觉设定条目有不与其他资产重复的执行锚点；分镜静态视觉锚点逐字进入冻结帧和对应运动正文。起点可见资产的执行锚点全部进入冻结帧，过程/终点资产至少进入运动正文；真实参考图不豁免同镜其他资产；项目专名、`same character`、`与上一镜相同` 或 `保持已有外观` 不算视觉依据；
- 冻结关键帧只投影所声明的边界：默认起点；外部工作流确实需要时才增加终点帧；
- 可复制关键帧正文不含 ID、文件路径、工作流状态或工艺说明。

结构检查失败时点名文件、场景或镜头 ID、冲突事实和负责 owner，不回显整段私有文本。

## 需要语义审查

下列问题不能靠字段数量或脚本替代：

- 镜头是否守住来源原意和观众知情时机；
- 重复覆盖是否真的增加新的反应、证据或理解；
- 景别、机位、运动和切点是否由注意、压力、关系、空间或节奏变化驱动；
- 多人调度是否让主要关系和注意交接可读；
- 证据文字、决定性手部动作、脸与眼神是否会被遮挡；
- 关键帧是否既可冻结，又保留镜头下一步需要发生的动作空间。
- 可见资产清单是否真的完整：特别复查大全景小人物、遮挡/背影人物、镜中/屏中人物、过程才入画的道具；同时确认画外声音没有被误报成可见资产。
- 执行锚点在当前景别是否可见而不过度：远景用剪影、发型、服装色块，近景才使用脸部细节；不能为满足逐字检查写入当前画面根本看不见的特征。

关键帧数量、镜头数量、景别比例和目标时长差值都不是通用质量门槛。审查 finding 必须给出位置、
证据、影响和必须达到的修订结果，不能只说“镜头不够电影感”或“看起来像 AI”。
