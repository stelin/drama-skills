...
| SHT-22 | structural_invariant | Each shot declares the complete set of reusable visual-setting assets that are actually visible at its start, during motion, or at its end. Every declared item resolves to the exact visual-setting heading and is covered by an image-prompt item with an explicit asset mapping or by a textual visual-basis entry; their union neither omits a visible asset nor binds an absent one. A small or background character remains in scope when the frame asks the audience or execution to recognize that identity or Look. |
| SHT-23 | structural_invariant | Each visual-setting item owns one prompt-language execution anchor. A shot owns one static visual anchor that its frozen prompt and matching motion copy unchanged. Without real input images, that anchor carries every start-visible item's execution anchor, and the motion prompt carries every item that appears at any point; a project name, “same as previous”, or “keep the existing look” cannot stand in for visual facts the execution never received. |
