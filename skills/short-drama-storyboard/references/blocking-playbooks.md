...
写成 `<working side>` 这类占位等于没写。判断依据是：换一个人来拍下一镜，他能不能只读这
三行就把机位放对。
...
同一个双人镜头可以容纳多轮对话。只有注意、解释或权力真的改变时才需要切镜。
...
**`SHT-13 · reviewed_invariant`**：多人对峙先画一张**有方向的关系图**：谁此刻保护/协作/施压/观察谁，这条关系由哪个
剧本动作或信息状态支持，再把它翻成站位、身体朝向、视线、距离和行动线。关系图只对
...
当前时间/回合、区域、控制者、运动方向、目标路线与可用反击；若某项不适用就省略，不为
填表虚构数据。
...
镜头结束边界是事实来源；关键帧只转写镜头开始边界；运动规格只实现这段过程。
