...
修订时先判断镜头的叙事任务是否仍是同一个，再决定保留、创建或 retire。

## 身份规则

- **重排**：只改变顺序，镜头目的与边界职责不变时保留原 ID；刷新依赖顺序与相邻边界。
- **插入**：新增一个独立注意或戏剧职责时创建新 ID；不要给后续所有镜头重新编号。
- **内容修订**：同一镜头目的下调整构图、blocking 或边界，保留 ID 并发布新 hash。
...
内的前身与 retired 关系使用稳定 ID，不伪造指向本文件自身的 hash。

## 每次拆分、合并或重排后的对账

重新核对 coverage 的每条 `shot_refs`、场次视觉计划转写、关键帧、motion、容器成员、相邻
start/end boundary、总时长与审查 hash。旧 ID 保留在 lineage / retire 记录中用于解释历史，
