...
来自对剧本节拍、分镜、关键帧与视频提示词如何衔接的案例归纳，示例均为合成改写。
...
需要时才补充时间段意图，完整动作用词由视频提示词环节编写。`start_boundary` 可以是
静止关键帧，也可以是上游明确的运动中瞬间，不能把“静帧输入”误写成故事必须静止。

## 一、通用原则

1. **一镜一职责**（SHT-03 的生产表述）：每镜有一个主导叙事任务。台词、反应与动作
   可同镜存在，只要镜头时长、表演负担和注意顺序可执行；过载时按行动转向、听者后果或
   动作阶段拆镜，不按句号机械切镜。
...
- 分镜号 = `场序-镜序`。**场序按 clip 内"地点组"的先后顺序排**：换地点才递增，同地点的
...
| 剧本形态 | 拆法 |
|---|---|
| 一个节拍 = 一句台词 + 一个动作 | 若同一镜头目的和边界可承担，可保留一镜；不是等式 |
| 一个节拍含多次行动转向或听者后果 | 按议程变化、听者反应与可执行语速拆镜；不使用固定句数/字数配额 |
| 一个节拍是爆发动作 | 若单镜过载，可按蓄力/接触/结果等动作阶段拆，切点绑定精确状态 |
| 一个节拍是纯反应/沉默 | 只有承担反应或选择后果时独立成镜；停留由后果与节奏决定 |
...
- 同框的并行动作只放有来源的变化：他人反应、环境压力或持续运动锚点可以
  帮助画面成立，但没有叙事/物理依据时宁可保持静止，不为满足配额发明微动态；
- 只有已接受的时长或制作方案要求时才写秒数、角度或位移；它们是当前镜头选择，
  不是跨项目动作质量公式；
- 台词保持精确引用与相对动作顺序；是否把动作展开成更多阶段取决于时长和表演负担。

**质量问题**：不改动台词和上游顺序时，这个镜头是否仍明确谁承担信息、谁接收
...
| 观众立场改变 | 观众此前和谁知道得一样多、此刻为何要靠近或疏离？ | 保持原视点、换到另一方的主观视角、拉开关系或有意拒绝切换 |
| 空间压力改变 | 谁控制入口、距离、路径或画外威胁？地理是否已经建立？ | 调整机位、跟位移、用遮挡，或保持固定让调度自己改变关系 |
| 力学动作需要理解 | 蓄力、接触、结果中哪些必须连续看清，哪些切点已有精确状态？ | 单镜保持、按动作阶段拆、补细节镜头或反应镜头 |
...
前两栏是镜头拥有的调度事实，关键帧只转写不覆盖；焦段意图是关键帧自己的选择，因为它
...
- **动作戏**：观众必须理解哪条路径、接触和结果？人物选择与代价落在哪里？先保证
  地理和道具归属，再决定连续保持、拆动作阶段或切反应。
- **情绪戏**：触发是什么，人物如何可见地处理，它促成什么选择或抑制？沉默、对白、
  手部、空间距离和不切镜都可承担，不把情绪名自动匹配到特写或推拉摇移。
