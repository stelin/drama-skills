...
| 抽样快速评估一本书值不值得拆 | `$short-drama-novel-analyze` | 全量拆解前先按抽样判断改编密度与风险 | 结论限于抽样范围，不产候选集条目 |
...
| 单题材执行细节 | `$short-drama-develop` | 已确定题材、要落到场面质感与做法 | 一次只读一张卡；卡片不产生可阻断约束 |
...
| Look Development 与风格帧 | `$short-drama` | 把视觉方向变成人物、地点和高压力代表帧的可观察规则 | `$short-drama-image-prompts` 只转写提示词；风格不接管身份、地理或剧情 |
| 场景与可表演行动 | `$short-drama-write` | 把单集节拍落实成场景 | 不决定分镜景别和运镜 |
| 对白行动与人物声音 | `$short-drama-write` | 写、改或诊断对白 | 不按句长、口头禅数量判权力 |
...
| 身份、变体与复用 | `$short-drama-assets` | 决定复用、新变体、新身份或待定 | 相机角度通常不是资产变体 |
| 资产图片提示词 | `$short-drama-image-prompts` | 把已接受资产转写为单帧可见规格 | 不包含有先后顺序的剧情动作 |
...
| 表演动作与时长 | `$short-drama-video-prompts` | 动作、台词与结果可能超载 | 不用每秒动作数设通用阈值 |
...
| 制作形态翻译 | `$short-drama` | 把实拍、二维、三维、水墨等方向转写到各阶段 | 风格名或模型代码不是生产决策 |
