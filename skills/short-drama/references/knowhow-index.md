...
| 血亲、年龄阶段、变异形态与派生资产 | `$short-drama-assets` | 一个人物的身份锚点部分来自另一个已确认人物 | 只认显式 `派生自` 声明，不从名字或关系标签推断；不改写身份与 Look 分层 |
| 观看契约（电影长片 / 横屏剧集 / 静态漫剧） | `$short-drama` | 项目不是竖屏短剧，竖屏取向的 craft_default 需要整体让位 | 契约是取向不是事实；不产生数字门槛，不改写镜头边界 |
| 角色设定板版式 | `$short-drama-image-prompts` | 要决定用面部基准三视板、服装发型核对板还是单视图 | 版式服务 reuse_job；比例数字只是 taste |
| 方言结构标记逐字对账 | `$short-drama-video-prompts` | 已接受档案声明了 H3 / Seedance 2.5 方言，正文带素材编号、切点时刻或时间戳 | 只核对方言文件明文规定的固定结构；通用路径只查标签不越界 |
| 集时长账目与原生区间 | `$short-drama-storyboard` | creator-first 五文档要加总镜头时长、核对已接受档案的原生区间 | 差值只报告；档案未声明时区间检查不生效 |
