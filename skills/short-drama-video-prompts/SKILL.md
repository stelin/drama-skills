...
7. 声音同轨时，把允许的发声按来源、说话人、口语语言和顺序写成完整时间线；每句从原文首字到末字，事件外回到已声明的环境声或静默。完整时间线声明即为声音段落终点，其后不追加声音排除句。
8. 成稿时逐句把 `no/not/without + 动作或状态变化` 改写为当前持续状态或允许动作；直接改正文，不调用工具或生成检查产物。再检查不跨下一镜、不要求互斥动作、不让人物或道具瞬移变形。
9. 用户要整集就完成整集，镜头组只是内部批次。
