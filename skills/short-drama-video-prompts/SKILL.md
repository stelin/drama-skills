...
| 分段交付、字段职责、时长分配、交付路由与执行触发词 | [交付档案与字段语义](references/delivery-profile.md) |
...
5. **环境与声音 `environment_motion/audio`**：只写有剧情和连续性依据的环境运动、对白、画外音、
   ...
   才逐镜转写其进入、撤出、距离、留白和 bridge，不在 motion 再保存第二条 plan ref。
...
中间镜不能省略声音状态。任一处不一致先退回修正，不靠流畅用词掩盖。
...
信息时机、因果顺序、观众立场或表演所有权、可读文字、场尾结果、制作成本或回退路径的高影响例外。
未被总括的新增选择要么删除，要么补入接受摘要。

若创作者声明了多镜交付容器，一个容器可以按来源顺序收录若干连续镜头：此时切换只允许出现在成员镜头边界上，容器时长必须等于成员镜头已接受时长之和，镜头目的与逐镜可审查性不变。容器分层、成员资格与两级算术见 [delivery-profile.md](references/delivery-profile.md)。
