...
6. 把限制动作空间的已确认关系写成贯穿本镜的正向可见状态，让有序动作只发生在允许的身体/物件通道上；只有分镜明确改变关系时才写解除过程。
7. 声音同轨时，把所有允许的发声按来源、说话人、口语语言和顺序列成完整集合；每个发声事件从原文首字开始、到原文末字结束，事件外回到已声明的环境声或静默。
...
- 声音同轨且有对白/VO/OS 时，逐字文本、口语语言与发声边界一起写：已列事件构成本镜完整发声集合，间隙只保留已声明环境声或静默，不给执行端留下自补开场词、语气词或其他话语的空档。
- 上游已确认的持续关系在正文中写成贯穿动作的正向状态；不要只重复起点名词后又给角色一个与该关系冲突的动作。
