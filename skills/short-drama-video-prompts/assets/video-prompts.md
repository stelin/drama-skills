...
> 从<最少但够用的起点信息>开始。<按因果和物理顺序写主体动作>。<本镜确有表演变化时，写承担变化者可见的触发、选择或结果；否则删除本句。>摄影机<有动机地固定或移动，并写清节奏和终点>。<必要环境运动>。对白/声音：<逐字文本/引用、表演方式、声源与层级>。在<已确认时长>内<安排节奏>，最终<逐项实现已确认终点，不写下一镜>。
