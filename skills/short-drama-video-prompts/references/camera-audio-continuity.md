...
同轨生成声音时，逐字正确还不够：本镜列出的所有对白、VO、OS、必要非语言发声和 SFX 共同组成
完整事件集合。每句台词标明口语语言，并用“环境声/静默 → 原文首字至末字 → 环境声/静默”限定
发声窗口；两句之间也回到已声明底声。它描述完整时间线，不依赖供应商专属禁词。
...
- 同轨正文声称列出了完整发声集合，却留下未定义的语音开场/收尾，或允许集合外话语与逐字台词并存。
