...
| VID-25 | structural_invariant | Under an accepted dialect, the fixed structural tokens of a copyable prompt reconcile byte-for-byte with the documents that derive them: material indices (`<Picture N>`, `@图片N`) stay within the shot's same-type REF `顺序`, container cut marks (`[Shot k] At mm:ss.mmm`) equal the cumulative accepted member durations, timestamp ranges are contiguous and end at the accepted duration, and fenced dialogue is verbatim screenplay text. The check reads only structures the dialect file spells out. |
| VID-26 | structural_invariant | When the creator turns on dialogue coverage, every dialogue, VO and OS line of `剧本.md` appears verbatim in some `MOTION-...` copyable prompt (same-pass audio) or shot sound field, or is explicitly omitted in `分镜.md` with a reason; the switch is off by default so a static comic drama is not forced to carry motion prompts. |
