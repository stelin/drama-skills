...
| VID-03 | structural_invariant | Choose text-to-video only when no real input image exists, and image-to-video only when the matching storyboard declares readable REF inputs. Copy those REF declarations and the storyboard-owned static visual anchor unchanged. Both routes carry every declared visible asset's minimum execution anchor before that asset enters the frame; image-to-video may omit richer repetition actually carried by a real reference frame, but a REF for one asset or composition never exempts the whole shot. A project name, “same character”, or “keep the existing look” is not a visual anchor. |
