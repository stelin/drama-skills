...
| Dialogue/audio | 逐字台词、说话人、口语语言、VO/OS/SFX 与本场声音策略是否保持？同轨时，全部允许发声是否形成封闭事件集合，首尾与间隙是否回到已声明底声/静默？ | 场景 ID、声音事实与正文 |
...
- 限制动作空间的持续关系从起点贯穿到终点；有序动作没有在无来源时解除接触、支撑、固定或占位关系；
...
- 「输入参考图」与「静态视觉锚点」分别逐字等于对应分镜；文生与图生视频都按分镜「可见资产」逐项核对，所有资产在第一次入画前都携带《视觉设定.md》的最小执行锚点，不能让一张构图/单资产参考图豁免整镜，也不能只写项目内人名或“同一个人”；
